...
for linea in $(cat 2013_2020.csv); do   #Abro el archivo csv y trabajo con cada una de sus líneas.
  suma=0    #Para cada linea voy a sumar los valores de las combinaciones ganadoras.
  for (( i=2; i<=7; i++ )) do   #Para cada linea del archivo trabajo con el rango de columnas de 2 a 7 ya que contienen los valores de las combinaciones ganadoras.
      num=$(echo $linea | cut -d "," -f$i)  #Obtengo el numero correspondiente a la columna i.
      let suma=$suma+10#$num
  done
  if [ $suma -eq $1 ]; then   #Si la suma de los valores coincide con el parámetro ingresado:
    let cant=$cant+1    #Actualizo la cantidad de ocurrencias.
    if [ "$fecha1" == "" ];then
      fecha1=$(echo $linea | cut -d "," -f1)   #Asigno la fecha de la primer ocurrencia.
    else
